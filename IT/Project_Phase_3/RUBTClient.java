...
public class RUBTClient {
					...
					for (int i = 0; i < pieceCount; i++) {
						...
						for (int j = 0; j < 2; j++) {
							...
							if (messageID == 7) {
								...
								byte[] currentPiece = Arrays.copyOfRange(block, 0, decodedTorrentByteFile.piece_length/2);
	...
	/*public static void writeToFile(byte[] completePieces) {
	...
	public static Peer findFastestPeer(ArrayList<Peer> peerList) {
		System.out.println("Getting fastest peer");
		System.out.println();

		Peer fastestPeer = null;
		
		float avgRTT = 0;
		float lowestAvgRTT = 0;
		float totalTime = 0;

		for (int k = 0; k < peerList.size(); k++) {
			Peer tempP = peerList.get(k);

		    totalTime = 0;
			avgRTT = 0;

			System.out.println("Sending 10 Ping Requests to " + tempP.ip);
			for (int i = 0; i < 10; i++) {
				try {
					InetAddress inet = InetAddress.getByName(tempP.ip);
					...
					if (inet.isReachable(5000)) {
						...
						System.out.println(tempP.ip + " NOT reachable.");
					}
				} catch (Exception e) {
					System.out.println("Can't find RTT for peer");
				}
			}

			avgRTT = totalTime / 10;
			System.out.println(tempP.ip + " Total RTT for 10 pings: " + totalTime + "ms");
			System.out.println(tempP.ip + " Average RTT for 10 pings: " + avgRTT + "ms");
			System.out.println();

			if (k == 0) { //set the first peer as fastest
				lowestAvgRTT = avgRTT;
				fastestPeer = tempP;
			}

			if (lowestAvgRTT > avgRTT) {
				lowestAvgRTT = avgRTT;
				fastestPeer = tempP;
			}
		}
		
		System.out.println("Fastest peer is " + fastestPeer.ip);
		System.out.println();
		return fastestPeer;
	}
}
